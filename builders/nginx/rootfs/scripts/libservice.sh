...
#########################
cronic_start() {
    # Get all .cron files in the HOME directory
    cron_files=$(ls -1 "${HOME}"/*.cron 2>/dev/null)

    # Check if any .cron files were found
    if [ -z "$cron_files" ]; then
        echo "No cron files found in ${HOME}"
        return 1
    fi

    # Iterate over each .cron file and start Supercronic for each
    for cron_file in $cron_files; do
        echo "Starting Supercronic for: $cron_file"
        supercronic -split-logs -json -passthrough-logs -quiet "$cron_file" > /proc/1/fd/1 2>&1 &
    done
}
########################
...
########################
# Generate a cronic configuration file for a given service
# Arguments:
#   $1 - Service name
#   $2 - Command
# Flags:
#   --schedule - Cron schedule configuration (default: @hourly)
# Returns:
#   None
#########################
generate_cronic_conf() {
    local service_name="${1:?service name is missing}"
    local cmd="${2:?command is missing}"
    ...
    if "$clean"; then
        cat > "${HOME}/${service_name}.cron" <<EOF
${schedule} ${cmd}
EOF
    else
        echo "${schedule} ${cmd}" >> "/${HOME}/${service_name}.cron"
    fi
}
...
#########################
log_rotate_run() {
    local logrotate_conf_dir="${1:-$HOME}"
    if [[ -x "/usr/sbin/logrotate" ]]; then
        if [[ -f "${logrotate_conf_dir}/logrotate.conf" ]]; then
            /usr/sbin/logrotate ${logrotate_conf_dir}/logrotate.conf -s /tmp/logrotate.status
        fi
    else
        false
    fi
...
generate_logrotate_conf() {
    ...
    local rotations="90"
    local extra=""
    local logrotate_conf_dir="${3:-$HOME}"
    local var_name
    # Parse optional CLI flags
    shift 2
    while [[ "$#" -gt 0 ]]; do
...
remove_logrotate_conf() {
    local service_name="${1:?service name is missing}"
    local logrotate_conf_dir="${2:-$HOME}"
